...
//user click on the category and it show tutor profile and after mentee confirm it will navigate to payment
  ...
  const fetchMentorInfo = async () => {
    try {
      const res = await fetch(
        `https://64b659dfdf0839c97e156749.mockapi.io/api/v1/mentor/${mentorId}`,
        {
          method: "GET",
          headers: {
      ...
      <Flex
        ...
        <Box w={"50%"}>
          ...
          <Text
            size="md"
            mb={2}>
            About Me: {mentorInfo?.bio}
          </Text>
          <Heading
            size={"sm"}
            m={"7px"}>
            Date of birth: {mentorInfo?.dateOfBirth}
          </Heading>
          <Heading
            size={"sm"}
            m={"7px"}>
